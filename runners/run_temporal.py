...
import models.temporal_model
import models.encoder
import models.decoder
import losses.loss as losses
import metrics.metric as metrics
import utils.train_util as train_util
from utils.build_vocab import Vocabulary
from runners.base_runner_temporal import BaseRunner
...
class Runner(BaseRunner):
    ...
    def _forward(self, model, batch, mode, **kwargs):
        ...
        attn_feats = convert_tensor(attn_feats.float(),
                                    device=self.device,
                                    non_blocking=True)
        temporal_label = convert_tensor(temporal_label.long(),
                                        device=self.device,
                                        non_blocking=True)
        input_dict = {
