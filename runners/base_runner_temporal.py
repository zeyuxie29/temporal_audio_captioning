...
class BaseRunner(object):
    ...
    def _get_dataloaders(self, config):
        ...
        if "train" not in data_config:
            ...
            train_dataset = ac_dataset.CaptionDatasetTemporal(
                raw_audio_to_h5,
                fc_audio_to_h5,
                attn_audio_to_h5,
                caption_info,
            ...
            val_audio_to_label = {}
            for item in data["val"]["caption_info"]:
                val_audio_to_label[item["audio_id"]] = item["sed_label"]
            """
            val_audio_to_label = {}
            for item in data["val"]["caption_info"]:
                max_label = 0
                for i in item["captions"]:
                    if i["time_label"] > max_label:
                        max_label = i["time_label"]
                audio_id =  item["audio_id"]
                if audio_id not in val_audio_to_label.keys() or max_label > val_audio_to_label[audio_id]:
                    val_audio_to_label[audio_id] = max_label
            """
            val_dataset = ac_dataset.CaptionEvalDatasetTemporal(
    ...
    def _eval_prediction(key2refs, key2pred, scorers, pretokenized=False, per_audio=False):
        ...
        for scorer in scorers:
            if type(scorer) == dict: # dict->fense                                                                                  
                for name, sub_scorer in scorer.items():                                                                             
                    score, scores = sub_scorer.compute_score(key2refs, key2pred)                                                    
                    output[name] = score 
                    if per_audio:
                        output["per_audio"][name] = dict(zip(
        ...
        return output
    """
    def eval_evaluate_seeds(self, experiment_path, task,  **kwargs):
        seeds = [1,10,100,1000]
        for save_type in ["last"]:#, "beat", "swa"]:
            for seed in seeds:
                self.evaluate(experiment_path + "/seed_" + str(seed), task, save_type=save_type, caption_output=f"{save_type}_output.json", score_output=f"{save_type}_scores.txt", method="beam", **kwargs)
        return experiment_path

    def eval_evaluate(self, experiment_path, task, **kwargs):
        for save_type in ["last"]:#, "beat", "swa"]:
            self.evaluate(experiment_path, task, save_type=save_type, caption_output=f"{save_type}_output.json", score_output=f"{save_type}_scores.txt", method="beam", **kwargs)
        return experiment_path
    """ 
    def evaluate(self,
                 experiment_path: str,
                 ...
                 caption_file: str,
                 save_type: str = "last",
                 caption_output: str = "last_output.json",
                 score_output: str = "last_scores.txt",
                 method: str = "beam",
                 **kwargs):
        a = caption_file
        prefix = a.split('/')[-2].split('_')[0] + '_' +  a.split('/')[-1].split('.')[0]
        """kwargs: {'max_length': int, 'method': str, 'beam_size': int}"""
        eval_data = {
            "clotho": {
                "raw_feat_csv": "/mnt/lustre/sjtu/home/xnx98/work/AudioCaption/data/clotho_v2/eval/lms.csv",
                "fc_feat_csv": "/mnt/lustre/sjtu/home/xnx98/work/AudioCaption/data/clotho_v2/eval/clap_features/cnn14/fc.csv",
                "attn_feat_csv": "/mnt/lustre/sjtu/home/xnx98/work/AudioCaption/data/clotho_v2/eval/clap_features/cnn14/attn.csv",
                #"caption_file": "data/clotho_v2/eval/text.json",
            },
            "newdata_audiocaps": {
                "raw_feat_csv": "/mnt/lustre/sjtu/home/xnx98/work/AudioCaption/data/audiocaps/test/lms.csv",
                "fc_feat_csv": "/mnt/lustre/sjtu/home/xnx98/work/AudioCaption/data/audiocaps/test/panns_wavegram_logmel_cnn14_fc.csv",
                "attn_feat_csv": "/mnt/lustre/sjtu/home/xnx98/work/AudioCaption/data/audiocaps/test/panns_wavegram_logmel_cnn14_attn.csv",
                #"caption_file": "data/audiocaps/test/text.json",
            },
        }
        eval_data[task]["caption_file"] = caption_file
        ...
        for item in captions:
            audio_to_label[item["audio_id"]] = item["sed_label"]
        """
                max_label = 0
                for i in item["captions"]:
                    if i["time_label"] > max_label:
                        max_label = i["time_label"]
                audio_id =  item["audio_id"]
                if audio_id not in audio_to_label.keys() or max_label > audio_to_label[audio_id]:
                    audio_to_label[audio_id] = max_label
        """
        dataset = ac_dataset.CaptionEvalDatasetTemporal(
            raw_audio_to_h5,
            fc_audio_to_h5,
            attn_audio_to_h5,
            audio_to_label
        ...
        json.dump({"predictions": pred_data}, open(experiment_path / (prefix + caption_output), "w"), indent=4)
        ...
        if not zh:
            ...
            scorers.append({"Fense":Fense(), "FenseWoPenalty":Fense(penalty=0)})
            #scorers.append(Fense(penalty=0))
        scores_output = self._eval_prediction(key2refs, key2pred, scorers)

        with open(str(experiment_path / (prefix + score_output)), "w") as f:
            f.write("save_type: {}\n".format(save_type))
            spider = 0
